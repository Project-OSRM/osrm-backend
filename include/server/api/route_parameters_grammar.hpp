...
inline const auto continue_straight_rule =
    ...
    (x3::lit("default") | x3::bool_[([](auto &ctx) {
...
inline const auto steps_rule = x3::lit("steps=") > x3::bool_[([](auto &ctx) {
                                   x3::get<params_tag>(ctx).get().steps = x3::_attr(ctx);
                               })];

inline const auto geometries_rule =
    x3::lit("geometries=") > geometries_type[([](auto &ctx) {
        x3::get<params_tag>(ctx).get().geometries = x3::_attr(ctx);
    })];

inline const auto overview_rule =
    x3::lit("overview=") > overview_type[([](auto &ctx) {
        x3::get<params_tag>(ctx).get().overview = x3::_attr(ctx);
    })];
...
inline const auto waypoints_rule =
    x3::lit("waypoints=") >
    (size_t_ % ';')[([](auto &ctx) {
        x3::get<params_tag>(ctx).get().waypoints = x3::_attr(ctx);
    })];
