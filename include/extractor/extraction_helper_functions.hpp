...
inline const auto hh = x3::rule<struct hh_tag, unsigned>{"hh"} = uint2_p[([](auto &ctx) {
    if (x3::_attr(ctx) >= 24)
    {
        x3::_pass(ctx) = false;
        return;
...
inline const auto mm = x3::rule<struct mm_tag, unsigned>{"mm"} = uint2_p[([](auto &ctx) {
    if (x3::_attr(ctx) >= 60)
    {
        x3::_pass(ctx) = false;
        return;
...
inline const auto ss = x3::rule<struct ss_tag, unsigned>{"ss"} = uint2_p[([](auto &ctx) {
    if (x3::_attr(ctx) >= 60)
    {
        x3::_pass(ctx) = false;
        return;
...
inline const auto osm_time = x3::rule<struct osm_time_tag, unsigned>{"osm_time"} =
    (uint_p >> ':' >> uint_p >> ':' >> uint_p >> x3::eoi)[([](auto &ctx) {
        auto &a = x3::_attr(ctx);
        x3::_val(ctx) = boost::fusion::at_c<0>(a) * 3600 + boost::fusion::at_c<1>(a) * 60 +
                         boost::fusion::at_c<2>(a);
    })] |
    (uint_p >> ':' >> uint_p >> x3::eoi)[([](auto &ctx) {
        auto &a = x3::_attr(ctx);
        x3::_val(ctx) =
            boost::fusion::at_c<0>(a) * 3600 + boost::fusion::at_c<1>(a) * 60;
    })] |
    (uint_p >> x3::eoi)[([](auto &ctx) { x3::_val(ctx) = x3::_attr(ctx) * 60; })];

// Thhmmss (compact)
inline const auto alternative_time =
    x3::rule<struct alt_time_tag, unsigned>{"alternative_time"} =
    ('T' >> hh >> mm >> ss)[([](auto &ctx) {
        auto &a = x3::_attr(ctx);
        x3::_val(ctx) = boost::fusion::at_c<0>(a) * 3600 + boost::fusion::at_c<1>(a) * 60 +
                         boost::fusion::at_c<2>(a);
...
inline const auto standard_date = x3::rule<struct std_date_tag, unsigned>{"standard_date"} =
    (x3::uint_ >> x3::char_("Dd"))[([](auto &ctx) {
        x3::_val(ctx) = boost::fusion::at_c<0>(x3::_attr(ctx)) * 86400;
    })];

inline const auto standard_week = x3::rule<struct std_week_tag, unsigned>{"standard_week"} =
    (x3::uint_ >> x3::char_("Ww"))[([](auto &ctx) {
        x3::_val(ctx) = boost::fusion::at_c<0>(x3::_attr(ctx)) * 604800;
    })];

// Collapse date+time into single value
inline const auto date_time_duration =
    x3::rule<struct dt_dur_tag, unsigned>{"date_time_duration"} =
    (-standard_date >> -standard_time)[([](auto &ctx) {
        auto &a = x3::_attr(ctx);
        unsigned d = 0, t = 0;
...
inline const auto time_or_datetime =
    x3::rule<struct tod_tag, unsigned>{"time_or_datetime"} =
