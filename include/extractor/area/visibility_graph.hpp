...
bool VisibilityGraph::visible(const Vertex *observer,
                              ...
                              const std::vector<Segment> &tau)
{
    // Check if observer -> w is inside the polygon immediately before intersecting the edge.
    // This condition also checks if the ray falls entirely outside the outer ring.
    // if observer -> w intersects the interior of the obstacle of which w is a vertex, locally at w
    // the w is not visible
    if (in_cone(w->next, w, w->prev, observer))
    {
