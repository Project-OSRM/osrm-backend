...
{
    if (entry.duration == MAXIMAL_EDGE_DURATION) return util::json::Value(util::json::Null());
...
{
    if (entry.distance == MAXIMAL_EDGE_DISTANCE) return util::json::Value(util::json::Null());
    return util::json::Value(util::json::Number(entry.distance));
}

template<typename PayloadType>
inline util::json::Value empty_encoder(const PayloadType & entry) 
{
...
template<>
inline PAYLOAD_ENCODER<DurationDistancePayload> get_encoder<DurationDistancePayload>(TableOutputField component) 
{
    switch(component)
    {
        case DURATION:
        ...
        default:
            throw util::exception("Internal Error: Encountered unknown output component type: " + component);
    }
} 
...
inline PAYLOAD_ENCODER<DurationPayload> get_encoder<DurationPayload>(TableOutputField component) 
{
    switch(component)
    {
        ...
        case DISTANCE:
            throw util::exception("Internal Error: Requested distance, but osrm is not configured to provide distances.");//TODO: Graceful error handling
        default:
            throw util::exception("Internal Error: Encountered unknown output component type: " + component);
    }
} 
