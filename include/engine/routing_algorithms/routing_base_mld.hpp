    ...
    namespace osrm::engine::routing_algorithms::mld
{
    ...
    //   * use partition.GetQueryLevel to find the node query level based on source and target
    //   phantoms
    ...
    {
        return getNodeQueryLevel(
            partition, node, endpoints.source_phantom, endpoints.target_phantom);
    }
                                     ...
                                     const std::vector<std::size_t> &phantom_indices)
    {
        ...
        // This is equivalent to min_{∀ source, target} partition.GetQueryLevel(source, node,
        // target)
    ...
    inline void retrievePackedPathFromSingleManyToManyHeap(
        const SearchEngineData<Algorithm>::ManyToManyQueryHeap &heap,
        const NodeID middle,
        OutIter out)
    {
    ...
    inline void retrievePackedPathFromSingleHeap(
        const SearchEngineData<Algorithm>::QueryHeap &heap, const NodeID middle, OutIter out)
    {
        NodeID current = middle;
        NodeID parent = heap.GetData(current).parent;
    ...
    template <bool DIRECTION>
    inline PackedPath retrievePackedPathFromSingleHeap(
        const SearchEngineData<Algorithm>::QueryHeap &heap, const NodeID middle)
    {
        PackedPath packed_path;
    ...
    inline PackedPath retrievePackedPathFromHeap(
        const SearchEngineData<Algorithm>::QueryHeap &forward_heap,
        ...
        const NodeID middle)
    {
        ...
        auto packed_path =
            retrievePackedPathFromSingleHeap<FORWARD_DIRECTION>(forward_heap, middle);
                        ...
                        const Args &...args)
    {
        if (forward_heap.Empty() || reverse_heap.Empty())
        {
            return std::make_tuple(
                INVALID_EDGE_WEIGHT, std::vector<NodeID>(), std::vector<EdgeID>());
        }
