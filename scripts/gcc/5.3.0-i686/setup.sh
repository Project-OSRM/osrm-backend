...
if [ ! -f "${MASON_PREFIX}/${MASON_LIB_FILE}" ] ; then
    ...
    if [ $MASON_HOST_PLATFORM = "osx" ]; then
        URL="${URL_MASON_BINARIES}/osx-x86_64/gcc-5.3.0-i686.dmg"
        FILE="${MASON_ROOT}/.cache/osx-x86_64-gcc-5.3.0-i686.dmg"
    elif [ $MASON_HOST_PLATFORM = "linux" ]; then
        URL="${URL_MASON_BINARIES}/linux-$(uname -m)/gcc-5.3.0-i686.tar.bz2"
        FILE="${MASON_ROOT}/.cache/linux-$(uname -m)-gcc-5.3.0-i686.tar.bz2"
    fi
    if [ ! -f ${FILE} ] ; then
