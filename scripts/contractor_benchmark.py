"""
A benchmark to compare osrm-contract binaries.

This program lets you compare two or more `osrm-contract` binaries wrt. time, memory,
and number of generated edges.

The run subcommand will select the first binary and contract the dataset.  It will then
select the next binary dataset and do the same. The whole process will be repeated
`--runs` times.

Ideally the argument `logfiles` will have the same number of items as the argument
`binaries`.  If not, it is extended by repeating the last item.

The `report` subcommand can be used to analyze logfiles generated by earlier runs.

The `copy` subcommand can be used to copy an existing dataset into a new temp directory.
