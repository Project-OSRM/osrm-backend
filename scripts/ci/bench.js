...
function bootstrapConfidenceInterval(data, numSamples = 1000, confidenceLevel = 0.95) {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    for (let j = 0; j < dataLength; j++) {
      const randomIndex = Math.floor(RNG() * dataLength);
  ...
  means.sort((a, b) => a - b);
  const lowerBoundIndex = Math.floor((1 - confidenceLevel) / 2 * numSamples);
  const upperBoundIndex = Math.floor((1 + confidenceLevel) / 2 * numSamples);
  const mean = means.reduce((a, b) => a + b, 0) / means.length;
  const lowerBound = means[lowerBoundIndex];
  const upperBound = means[upperBoundIndex];

  return { mean, lowerBound, upperBound };
}

function calculateConfidenceInterval(data) {
  const { mean, lowerBound, upperBound } = bootstrapConfidenceInterval(data);
  const bestValue = Math.max(...data);
