...
# --- feature-dataset accumulating vector on routed -------------------------

# Vector option accepts repeated flag invocations and accumulates.
assert_output_matches "routed/--disable-feature-dataset repeated parses" \
    "Required files are missing" \
    "$ROUTED" \
    --disable-feature-dataset ROUTE_STEPS \
    --disable-feature-dataset ROUTE_GEOMETRY \
    "$NONEXISTENT"

# Single invocation with one value still works.
