...
BOOST_AUTO_TEST_CASE(the_block_counts_as_much_as_the_walls)
{
    ...
    // 5,2 is 2 from the wall below and 2 from the block above, and either answer is the
    // same number.
    const auto tie = clearance({5, 2}, f.rings);
    BOOST_CHECK_CLOSE(tie.distance, 2.0, TOLERANCE);

    // Just inside the block's shadow the block wins outright, so the answer is the
    // distance to it and not to the wall 3 away.
    const auto blocked = clearance({5, 3}, f.rings);
    BOOST_CHECK_CLOSE(blocked.distance, 1.0, TOLERANCE);
}

BOOST_AUTO_TEST_CASE(a_point_on_the_geometry_has_no_clearance)
{
    const Fixture f;
...
BOOST_AUTO_TEST_CASE(degenerate_rings_do_not_divide_by_zero)
{
    ...
    BOOST_CHECK(std::isfinite(at.distance));
