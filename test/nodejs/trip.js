...
import { OSRM, FBResult, data_path, mld_data_path, three_test_coordinates, two_test_coordinates } from './constants.js';

test('trip: trip in Monaco with flatbuffers format', (assert) => {
  assert.plan(2);
  const osrm = new OSRM(data_path);
  osrm.trip({coordinates: two_test_coordinates, format: 'flatbuffers'}, (err, trip) => {
    assert.ifError(err);
...
test('trip: trip in Monaco', (assert) => {
  assert.plan(2);
  const osrm = new OSRM(data_path);
  osrm.trip({coordinates: two_test_coordinates}, (err, trip) => {
    assert.ifError(err);
...
test('trip: trip in Monaco as a buffer', (assert) => {
  assert.plan(3);
  const osrm = new OSRM(data_path);
  osrm.trip({coordinates: two_test_coordinates}, { format: 'json_buffer' }, (err, trip) => {
    assert.ifError(err);
...
test('trip: trip with many locations in Monaco', (assert) => {
  assert.plan(2);
...
test('trip: throws with too few or invalid args', (assert) => {
  assert.plan(3);
  const osrm = new OSRM(data_path);
  assert.throws(() => { osrm.trip({coordinates: two_test_coordinates}); },
    /Two arguments required/);
  assert.throws(() => { osrm.trip(null, (err, trip) => {}); },
    /First arg must be an object/);
  assert.throws(() => { osrm.trip({coordinates: two_test_coordinates}, { format: 'invalid' }, (err, trip) => {}); },
    /format must be a string:/);
});

test('trip: throws with bad params', (assert) => {
  assert.plan(14);
  const osrm = new OSRM(data_path);
  assert.throws(() => { osrm.trip({coordinates: []}, (err) => {}); });
  assert.throws(() => { osrm.trip({}, (err, trip) => {}); },
    /Must provide a coordinates property/);
  assert.throws(() => { osrm.trip({
    coordinates: null
  }, (err, trip) => {}); },
  ...
  assert.throws(() => { osrm.trip(options, (err, trip) => {}); },
...
test('trip: routes Monaco using shared memory', (assert) => {
  assert.plan(2);
  const osrm = new OSRM();
  osrm.trip({coordinates: two_test_coordinates}, (err, trip) => {
    assert.ifError(err);
...
test('trip: routes Monaco with hints', (assert) => {
  assert.plan(5);
  const osrm = new OSRM(data_path);
  const options = {
    coordinates: two_test_coordinates,
...
test('trip: trip through Monaco with geometry compression', (assert) => {
  assert.plan(2);
  const osrm = new OSRM(data_path);
  const options = {
    coordinates: [three_test_coordinates[0], three_test_coordinates[1]]
...
test('trip: trip through Monaco without geometry compression', (assert) => {
  assert.plan(2);
  const osrm = new OSRM(data_path);
  const options = {
    coordinates: two_test_coordinates,
...
test('trip: trip through Monaco with speed annotations options', (assert) => {
  assert.plan(12);
  const osrm = new OSRM(data_path);
  const options = {
    coordinates: two_test_coordinates,
  ...
  osrm.trip(options, (err, trip) => {
    ...
    for (let t = 0; t < trip.trips.length; t++) {
      ...
      assert.ok(trip.trips[t].legs.every((l) => { return l.annotation.speed; }), 'every leg has annotations for speed');
      ...
      assert.notOk(trip.trips[t].legs.every((l) => { return l.annotation.duration; }), 'has no annotations for duration');
      assert.notOk(trip.trips[t].legs.every((l) => { return l.annotation.distance; }), 'has no annotations for distance');
      assert.notOk(trip.trips[t].legs.every((l) => { return l.annotation.nodes; }), 'has no annotations for nodes');
...
test('trip: routes Monaco with null hints', (assert) => {
  assert.plan(1);
  const osrm = new OSRM(data_path);
  const options = {
    coordinates: [three_test_coordinates[0], three_test_coordinates[1]],
...
test('trip: service combinations that are not implemented', (assert) => {
  assert.plan(1);
  const osrm = new OSRM(data_path);
...
test('trip: fixed start and end combinations', (assert) => {
  assert.plan(21);
  const osrm = new OSRM(data_path);
...
test('trip: trip in Monaco without motorways', (assert) => {
  assert.plan(3);
  ...
  osrm.trip(options, (err, response) => {
    assert.ifError(err);
...
test('trip: throws on disabled geometry', (assert) => {
  assert.plan(1);
  ...
  osrm.trip(options, (err, route) => {
    console.log(err);
    assert.match(err.message, /DisabledDatasetException/);
  });
});

test('trip: ok on disabled geometry', (assert) => {
  assert.plan(2);
  const osrm = new OSRM({'path': data_path, 'disable_feature_dataset': ['ROUTE_GEOMETRY']});
  const options = {
    steps: false,
...
test('trip: throws on disabled steps', (assert) => {
  assert.plan(1);
...
test('trip: ok on disabled steps', (assert) => {
  assert.plan(8);
  const osrm = new OSRM({'path': data_path, 'disable_feature_dataset': ['ROUTE_STEPS']});
  const options = {
    steps: false,
