...
void Storage::LoadLayout(DataLayout *layout_ptr)
{
    ...
    layout_ptr->SetBlockSize<extractor::ProfileProperties>(DataLayout::PROPERTIES, properties_size);
...
void Storage::LoadData(DataLayout *layout_ptr, char *memory_ptr)
{
    ...
    *checksum_ptr = hsgr_header.checksum;
    ...
    auto datasource_name_offsets_ptr = layout_ptr->GetBlockPtr<std::uint32_t, true>(
    ...
    auto datasource_name_lengths_ptr = layout_ptr->GetBlockPtr<std::uint32_t, true>(
    ...
    /* TODO: replace this */
    boost::filesystem::ifstream timestamp_stream(config.timestamp_path);
    std::size_t timestamp_size = io::readNumberOfBytes(timestamp_stream);
    /* END TODO */
