...
-- If we can't bike, then use the foot profile to perform
-- a robust check of whether we can walk (ie. push the bike)
  
function Bicycle:handle_alternative_mode(d)
  local specific = (d.access_tag == self.settings.access.tags[1])
  if d.alternative ~= false and specific ~= true then
    if self:try_alternative_mode(Foot,d) then
      d.mode = 'pushing_bike'
    end
  end
end
