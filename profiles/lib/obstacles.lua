...
function Obstacles.process_node(profile, node)
    ...
    if highway then
        ...
        if typ and typ ~= obstacle_type.barrier then
            local direction = node:get_value_by_key("direction")
            local weight = 0
            local duration = 0
            local minor = false

            if typ == obstacle_type.traffic_signals then
            ...
            if typ == obstacle_type.stop then
                if node:get_value_by_key("stop") == "minor" then
                    typ = obstacle_type.stop_minor
                end
                duration = 2
            end
            if typ == obstacle_type.give_way then
                duration = 1
            end
            obstacle_map:add(node, Obstacle.new(typ, obstacle_direction[direction] or obstacle_direction.none, duration, weight))
        end
    end
end
